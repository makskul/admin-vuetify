...
import { ChannelListType, simpleRequestType } from "@/types/content"
...
const channels = ref<ChannelListType[]>([])
  ...
  <VCard>
    ...
    <VTable>
      ...
      <tr>
        ...
        <th scope="col" class="text-left cell-small">Feed ID</th>
      ...
      <tbody>
      <tr v-for="item in channels">
        <td class="text-center"><VCheckbox :value="item.id"/></td>
        <td class="text-center position-relative" style="width: 190px; min-width: 180px;">
          <RouterLink :to="{ name: 'channels-edit-id', params: { id: item.id } }">
            <VImg :src="item.avatarUrl" height="100" class="img-bg mt-1 w-auto">
        ...
        <td class="text-center">
          <RouterLink :to="{ name: 'channels-edit-id', params: { id: item.id } }" class="inherit-link">
            {{ item.title }}
          </RouterLink>
        </td>
        <td class="text-center">{{ item.rating || 0 }}</td>
        <td class="text-center">{{ item.total || 0  }}</td>
        <td class="text-center">{{ item.totalSponsor || 0  }}</td>
        <td class="text-center">{{ item.totalCommon || 0  }}</td>
        <td class="text-end">
          <RouterLink
            :to="{ name: 'parsers-edit-id', params: { id: channel.id } }"
            class="inherit-link"
          >
            <VBtn
                icon
