...
import { CategoryListType, simpleRequestType } from "@/types/content"
...
const categories = ref<CategoryListType[]>([])
const { setCurrentPageQuery, rowPerPage, resetPage, currentPage, totalPage, total, paginationData } = usePagination()
      ...
      <tbody>
        <tr v-for="item in categories">
          <td class="text-center">
            <VCheckbox :value="item.id" />
          </td>
          <td class="text-center position-relative" style="width: 190px; min-width: 180px;">
            <RouterLink :to="{ name: 'categories-edit-id', params: { id: item.id } }">
          ...
          <td class="text-end">
            <RouterLink :to="{ name: 'categories-edit-id', params: { id: item.id } }" class="inherit-link">
              <VBtn
                  icon
                  size="x-small"
                  color="default"
