...
import {getTagsBase, getPornstarsBase, getCategoriesBase, getSynonymBase, getChannelsBase} from "@/Api/content"

const props = defineProps(['modelValue', 'itemsGetter', 'preSelect', 'chip', 'label'])
...
function selectHandler(selectValue: any) {
  const newModel = selectValue.map((value: string) => {
    return JSON.parse(JSON.stringify(memoryItems.value.filter((x: any) => x.id === value)[0]))
  })
  const uniqueArray = newModel.filter((a: any, i: any) => newModel.findIndex((s: any) => a.id === s.id) === i)
  emit('update:modelValue', uniqueArray)
  reloadSelect(uniqueArray)
}
...
<template>
  <div>
    <div class="chips-container mb-2" v-if="props.chip">
      <VChip
        class="mb-1 mr-1"
        color="primary"
        v-for="value in selectedItemsModel"
        :key="value.id"
        closable
        @click:close="removeFromModelById(value.id)"
      >
        {{ value.title }}
      </VChip>
    </div>
