  ...
  axios.post<LoginResponse>('auth/login', { email: email.value, password: password.value })
      .then(r => {
        if (r.status === 201) {
          ...
          localStorage.setItem('userAbilities', JSON.stringify(userAbility))
          ability.update([
            {
              "action": "manage",
              "subject": "all"
          ...
          localStorage.setItem('userData', JSON.stringify({ "id":1,"fullName":"John Doe","username":"johndoe","email":"admin@demo.com","role":"admin" }))
